...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    # shape -> (n_test,1) + (n_train,) - (n_test, n_train)
    # dist.shape = (num_test, num_train)
    sq_test_train = np.sum(X**2,axis=1,keepdims=True) + np.sum(self.X_train**2, axis=1)
    dists = np.sqrt(sq_test_train - 2* (X@self.X_train.T) )
